        ...
        for (int hl = 0; hl < 1; ++hl)
        {
            ...
            for (int i = 0; i < PARAM.globalv.nlocal; ++i)
            {
                for (int j = 0; j < PARAM.globalv.nlocal; ++j)
                {
                    ...
                    {                
                        int iic;
                        if(PARAM.inp.ks_solver=="genelpa" || PARAM.inp.ks_solver=="scalapack_gvx" || PARAM.inp.ks_solver=="pexsi" || PARAM.inp.ks_solver=="ntpoly")  // save the matrix as column major format
                        {
                            iic = mu + nu * pv->nrow;
                        }
                        else
